...
Read the `## Confirmed Intent` section from the spec file as locked input. If it's missing or empty, stop and invoke `interview-me` first — the confirmed intent is a prerequisite, not something to redo here.
...
## Confirmed Intent
[Locked input from `interview-me`. Do not edit here without re-running `interview-me`.]
...
Specs are read by the `plan` skill (which produces an implementation plan) and by implementation agents that execute against that plan. Write the spec rich enough to support both:

- Every six core areas filled — thin sections produce thin plans
- Success criteria testable — *"LCP < 2.5s on 4G"* not *"faster"*
- Boundaries explicit — Ask First and Never lists prevent silent scope creep mid-build

A thin spec produces a thin plan and a confused implementer.

# Spec: Per-Feature Slug Folders for Shepherd

## Confirmed Intent

- **Outcome:** Move shepherd's per-feature artifacts (`spec.md`, `plan.md`, `progress.md`) into `.shepherd/specs/<slug>/` folders so multiple feature specs can coexist; `standards.md` and `implement.md` stay project-wide at `.shepherd/` root.
...
- **Why now:** The flat `.shepherd/spec.md` encodes a one-feature-per-repo mental model that doesn't match his iterative workflow — a second feature's spec literally overwrites the first's.
...
- **Constraint:** Slug folders only — no new commands, no concurrent worktree orchestration, no per-feature `standards`/`implement`, no phase decomposition. shepherd stays single-entry-point and autonomous within a feature, gated on the existing human-approval checkpoints.
- **Out of scope:** Native concurrent-feature execution (Spec Kitty / `--worktree` pattern); phase-decomposed CLI commands (`/shepherd spec`, `/shepherd plan`); migration tooling for existing flat `.shepherd/` directories in old repos.

### Confirmed Residual Design Decisions

1. **Branch ↔ folder coupling:** The git branch is named identically to the slug folder (e.g., branch `001-user-auth` ↔ `.shepherd/specs/001-user-auth/`). Shepherd resumes the correct feature by reading the current branch name.
...
3. **Slug naming:** `###-kebab-case` (e.g., `001-user-auth`). Shepherd proposes a slug during the intent gate, derived from intent keywords; user can override before approval.
4. **Slug substitution at dispatch time:** The orchestrator (shepherd's main process) substitutes `<slug>` literally into subagent dispatch prompts before dispatch, resolving slug from `git branch --show-current` in the orchestrator's cwd. Subagents MUST NOT compute slug from `git branch --show-current` themselves — each subagent runs in its own git worktree with its own task-branch, NOT the feature branch.

### Phase 1 Preconditions

Before any slug operation, shepherd MUST verify:

1. **Git repository present:** `git rev-parse --is-inside-work-tree` exits 0. If not, refuse and instruct user to run `git init` first. Shepherd requires a git repository.
2. **Branch is checked out:** `git branch --show-current` returns a non-empty branch name. If empty (detached HEAD, bare clone), refuse and prompt user to checkout a branch (or run `git checkout main`) before invoking shepherd again.

### Slug Determination & Collision Handling

When proposing a slug, shepherd:
...
**SHA pinning at plan-approval.** When a plan is approved, shepherd captures the SHA of each project-wide file at that moment and records it in the slug folder's `plan.md` (as a "Pinned Project-Wide State" section). On resume, shepherd compares the pinned SHAs against the current files; if any drift, MUST warn the user before proceeding.

### Phase 3 Completion: PR Handoff

On Phase 3 completion, shepherd MUST:
...
Shepherd MUST NOT: open the PR itself; merge the feature branch to main; delete the slug branch (locally or remotely).
...
This "project" is the **shepherd skill** — a markdown SKILL.md plus supporting files. Not a code project; an agent-skill project.
...
# Frontmatter sanity
head -5 skills/shepherd/SKILL.md
...
grep -nE '\.shepherd/(spec|plan|progress)\.md' skills/shepherd/SKILL.md skills/shepherd/references/project-templates.md
# Expected: empty output. Any match means migration is incomplete.

# Dogfood artifact exists and is well-formed
test -f .shepherd/specs/001-per-feature-slug-folders/spec.md
test -f .shepherd/specs/001-per-feature-slug-folders/plan.md
```

Behavioral eval (the workflow-shaped gate per CLAUDE.md):

```bash
# Run from skills/shepherd/evals/iteration-2/ (to be created by plan)
# Mirrors iteration-1's structure: benchmark.md + per-eval with_skill/without_skill outputs.
```

## Project Structure

### Target `.shepherd/` layout (this is the refactor's deliverable)
...
### Files changed by this refactor (within `skills/shepherd/`)
...
| `references/project-templates.md` | 2 path references (`.shepherd/spec.md` → slug-scoped) | Subagent workflow template needs the new spec path |
...
| File | Reason |
|---|---|
| `evals/iteration-1/**` (existing content: benchmark.md, eval-artifact-contract, eval-intent-discovery, live-run) | UNCHANGED. Documents what shepherd *did* at iteration-1 with the flat layout. Rewriting falsifies history. The new `evals.json` snapshot is a NEW file ADDED beside them; this preserves iteration-1's coherence without rewriting its content. |
| Any existing `.shepherd/spec.md` in target repos | No migration tooling per Out of Scope. User manually `mv`s if they want. |
...
- **Name each rule once.** Per memory `feedback_no_shadow_canon_in_skills`: never warn against the dead convention you just replaced. After the refactor, SKILL.md MUST NOT contain lines like "do not write to `.shepherd/spec.md` — that path is deprecated." The file is written canonically forward.
...
- **Path references in skill bodies use the new slug-scoped form.** Example:
  ...
  `.shepherd/specs/<slug>/spec.md` as its first section.
  ```
  ...
  ```markdown
  Once the gate clears, write the Confirmed Intent block to
  `.shepherd/spec.md` (now `.shepherd/specs/<slug>/spec.md` after refactor).
  ```
...
Expected output: empty. Any match means a flat-layout path leaked through. This is the refactor's most likely failure: leaving a stale `.shepherd/spec.md` reference in an example or instruction.
...
- `eval-intent-discovery/with_skill/output.md` — expected: writes to `.shepherd/specs/<slug>/spec.md` after slug proposal
- `eval-artifact-contract/with_skill/output.md` — expected: 5-file contract restated as 2 project-wide + 3 per-slug
- `live-run/verdict.md` — captures whether a real shepherd invocation produces the slug-scoped layout

Coverage requirement: the slug-determination step (Phase 1 Step 1.5) MUST have an explicit eval that fails if shepherd writes to the flat path.

## Boundaries

### Always do

- Write per-feature artifacts to `.shepherd/specs/<slug>/{spec,plan,progress}.md` — never to `.shepherd/spec.md`, `.shepherd/plan.md`, `.shepherd/progress.md`.
...
- Renaming `.shepherd/` itself or moving it under a different top-level (e.g., `.agents/shepherd/`).
...
- Add a `.shepherd/CURRENT` pointer file — slug is derived from the git branch name, not a separate state file.
- Auto-convert existing flat `.shepherd/spec.md` in target repos to slug-scoped form (migration is out of scope).
- Edit `.agents/skills/shepherd/` directly — that's a symlink to `skills/shepherd/` and the source is what gets edited.
...
1. **Layout exists:** `.shepherd/specs/001-per-feature-slug-folders/spec.md` and `.shepherd/specs/001-per-feature-slug-folders/plan.md` exist in this (agents-skills) repo and are tracked by git. (The dogfood: the spec for the change lives in the structure the change creates.)
...
3. **Phase 1 of SKILL.md describes the slug-determination sub-step** (after the explicit-yes gate, before writing the spec file), including shepherd proposing a slug and user override.

4. **Phase 2 of SKILL.md reads the slug-scoped progress.md** — the orchestration loop's "Read progress.md + plan.md" node is updated to "Read `.shepherd/specs/<slug>/progress.md` + `.shepherd/specs/<slug>/plan.md`" where `<slug>` is resolved from the current git branch.
...
6. **Subagent dispatch prompts reference slug-scoped paths** — implementer, reviewer, and fix dispatch all reference `.shepherd/specs/<slug>/...` in their context blocks.

7. **`references/project-templates.md` updated:** the "Read `.shepherd/spec.md`" instruction in the subagent workflow template uses the slug-scoped form.
...
11. **Packaging check passes:** `python3 skills/creating-skills/scripts/quick_validate.py skills/shepherd` exits 0.

12. **Line count stays under 500:** `wc -l skills/shepherd/SKILL.md` reports < 500 (CLAUDE.md gate).
...
14. **Phase 3 PR handoff exists:** Phase 3 PR handoff section exists; shepherd pushes the branch and prints the gh command; does NOT merge to main or delete the branch.
...
1. **Slug determination during intent gate (Assumption 1):** I folded slug acceptance into the existing intent restate gate — shepherd proposes `###-kebab` derived from intent keywords, user confirms with the same "yes" that confirms the intent. Alternative: a separate gate ("Confirm slug? [001-user-auth] yes/override"). The combined gate keeps Phase 1 step count stable; the separate gate is more explicit. **Default: combined.** Resolved: see Slug Determination & Collision Handling.

2. **Worktree-to-feature-branch merge (Assumption 2):** Phase 2's subagent worktrees currently merge to `main` (SKILL.md line 103). After this refactor they should merge to the feature branch (`001-...`). Open question: does this change break the existing dispatch pattern? My read says no — the merge target is just `git merge` into wherever the orchestrator is running, and shepherd runs on the feature branch. **Default: merge to feature branch; main only at human PR merge.** Resolved: see Phase 3 Completion: PR Handoff.

3. **Resume-on-`main` behavior (Assumption 3):** If user invokes shepherd while on `main` (no matching slug folder), what does shepherd do? Options: (a) treat as fresh invocation, propose new slug, create branch; (b) refuse and prompt user to checkout the feature branch first. **Default: (a) treat as fresh** — matches the spec-kit pattern where invocation creates the branch. Resolved: see Phase 1 Preconditions.

4. **First-time-on-repo setup:** First shepherd invocation on a fresh repo has no `.shepherd/standards.md` or `.shepherd/implement.md` yet. Phase 1 Step 3 creates them. Open question: do they get created on the feature branch (`001-...`) and merged via the first PR, or directly on `main` before the feature branch is created? **Default: created on the feature branch alongside the first feature; landed on `main` via the first PR.** Cost: standards.md isn't on main until the first feature lands. Benefit: keeps everything in one PR. Resolved: see Cross-Feature Coordination for Project-Wide Files.
...
*This spec lives at `.shepherd/specs/001-per-feature-slug-folders/spec.md` — using the very directory structure it specifies. The dogfood is the proof that the new layout works at the repository it's being added to.*

...
def source_surface_eval(root: Path, sources: dict[str, str]) -> dict:
    ...
    assertions = {
        ...
        "setup spine includes acceptance readiness before plan": "4. Acceptance Pipeline Readiness -> 5. Plan -> 6. Setup Close" in skill,
        "acceptance readiness blocks planning": "Do not invoke `plan`, present a final plan, or start implementation while acceptance-pipeline readiness is unrecorded or blocked." in skill,
        "acceptance readiness records missing machinery": "record `implementer task required` for normal acceptance components and `architect task required` for mutation hardening" in skill,

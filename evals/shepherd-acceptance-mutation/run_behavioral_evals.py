...
def source_surface_eval(root: Path, sources: dict[str, str]) -> dict:
    ...
    assertions = {
        ...
        "setup completion gates exist": "## Step Completion Gates" in skill and "Required Artifact" in skill and "Do not advance by intent or chat summary" in skill,
        "setup gates require user signoff before autonomy": "`plan` returns `READY` and the user signs off the plan." in skill and "Autonomous milestone execution may start." in skill,
        "progress is initialized before setup actions": "Create `.shepherd/progress.md` from `references/project-templates.md`, then invoke `interview-me`" in skill and "### 5. Setup Close" in skill and "### 6. Progress Init" not in skill,
        ...
        "no obsolete role-owned quality section": "### 4. Role-Owned Quality Work" not in skill,
